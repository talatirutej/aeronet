...
// ── Section label ─────────────────────────────────────────────────────────────
function SL({n,t}){return(<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}><span style={{fontSize:10,fontWeight:600,color:'var(--blue)',fontFamily:"'IBM Plex Mono',monospace"}}>{n}</span><div style={{flex:1,height:0.5,background:'var(--sep)'}}/><span style={{fontSize:10,fontWeight:600,color:'rgba(235,235,245,0.28)',letterSpacing:'0.08em',textTransform:'uppercase'}}>{t}</span></div>)}
...
export default function Views2DPage() {
  ...
  const toggleBtn = (label, val, set) => (
    <button key={label} onClick={()=>set(p=>!p)} style={{
      ...
      color: val?'var(--blue)':'rgba(235,235,245,0.35)',
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px 14px' }}>
            ...
            {preview ? (
              <div style={{ position:'relative' }}>
                ...
                <div style={{ position:'absolute', bottom:6, left:0, right:0, textAlign:'center' }}>
                  <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', background:'rgba(0,0,0,0.55)', padding:'2px 10px', borderRadius:20, fontFamily:"'IBM Plex Sans',sans-serif" }}>click to change</span>
                </div>
              </div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10, padding:'28px 16px', flex:1 }}>
                <div style={{ width:44, height:44, borderRadius:12, background:'var(--bg2)', border:'0.5px solid var(--sep)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(235,235,245,0.3)" strokeWidth="1.5">
                    <rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/>
                  </svg>
                </div>
                <span style={{ fontSize:12, color:'rgba(235,235,245,0.35)', fontFamily:"'IBM Plex Sans',sans-serif", textAlign:'center' }}>Drop a vehicle photo</span>
                <span style={{ fontSize:10, color:'rgba(235,235,245,0.2)', fontFamily:"'IBM Plex Mono',monospace" }}>JPG · PNG · WEBP</span>
              </div>
            )}
          </div>
          ...
          {file && (
            <div style={{ ...card, padding:'8px 12px', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
              <span style={{ fontSize:11, color:'rgba(235,235,245,0.45)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1, fontFamily:"'IBM Plex Sans',sans-serif" }}>{file.name}</span>
              <span style={{ fontSize:10, fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.2)', marginLeft:8, flexShrink:0 }}>{(file.size/1024).toFixed(0)} KB</span>
            </div>
          )}
          ...
          <button onClick={run} disabled={!file||isRunning} style={{
            ...
            color: !file||isRunning?'rgba(235,235,245,0.3)':'#fff',
          ...
          {error && (
            <div style={{ ...card, padding:'9px 12px', background:'rgba(255,69,58,0.08)', border:'0.5px solid rgba(255,69,58,0.3)', color:'var(--red)', fontSize:12, marginBottom:12, fontFamily:"'IBM Plex Sans',sans-serif" }}>{error}</div>
          )}
          ...
          {geo && (
            <>
              ...
              <div style={{ ...card, padding:'10px 12px', marginBottom:14 }}>
                {[['Type',geo.bodyType],['Aspect',geo.aspectRatio.toFixed(2)],['Hood',(geo.hoodRatio*100).toFixed(0)+'%'],['Cabin',(geo.cabinRatio*100).toFixed(0)+'%'],['Boot',(geo.bootRatio*100).toFixed(0)+'%'],['WS rake',geo.wsAngleDeg.toFixed(0)+'°'],['Rear drop',(geo.rearDrop*100).toFixed(0)+'%']].map(([k,v])=>(
                  <div key={k} style={{ display:'flex', justifyContent:'space-between', fontSize:11, padding:'3px 0', borderBottom:'0.5px solid rgba(255,255,255,0.04)' }}>
                    <span style={{ fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.28)' }}>{k}</span>
                    <span style={{ fontFamily:"'IBM Plex Mono',monospace", color:'var(--blue)' }}>{v}</span>
                  </div>
                ))}
                <div style={{ marginTop:8 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, fontFamily:"'IBM Plex Mono',monospace", marginBottom:4 }}>
                    <span style={{ color:'rgba(235,235,245,0.25)' }}>confidence</span>
                ...
                <div style={{ marginTop:8 }}>
                  <div style={{ fontSize:8, fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.15)', marginBottom:3 }}>VERTICAL EDGE DENSITY</div>
              ...
              <div style={{ ...card, padding:'14px', display:'flex', flexDirection:'column', alignItems:'center', marginBottom:14 }}>
                <CdGauge cd={cd}/>
                <div style={{ fontSize:10, fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.2)', marginTop:5 }}>estimated · {geo.bodyType}</div>
              </div>
            </>
          )}
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 12px', borderBottom:'0.5px solid var(--sep)', flexShrink:0, background:'rgba(0,0,0,0.4)', flexWrap:'wrap' }}>
          ...
          <div style={{ display:'flex', gap:2 }}>
            {VIEWS.map(v=>(
              <button key={v.id} onClick={()=>setActiveView(v.id)} style={{
                ...
                color: activeView===v.id?'var(--blue)':'rgba(235,235,245,0.38)',
          ...
          {geo && activeView==='top' && (
            <div style={{ display:'flex', alignItems:'center', gap:8, marginLeft:4 }}>
              <span style={{ fontSize:11, color:'rgba(235,235,245,0.35)', fontFamily:"'IBM Plex Sans',sans-serif" }}>Yaw</span>
              ...
              <span style={{ fontSize:11, fontWeight:600, color:'var(--blue)', fontFamily:"'IBM Plex Mono',monospace", width:28, textAlign:'right' }}>{yawAngle>0?'+':''}{yawAngle}°</span>
            </div>
          )}
          ...
          <button onClick={exportSVG} style={{
            ...
            color:'rgba(235,235,245,0.38)', fontSize:11, cursor:'pointer',
            ...
            onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--sep)';e.currentTarget.style.color='rgba(235,235,245,0.38)'}}>
            Export SVG
          </button>
        </div>
          ...
          {!geo ? (
            <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:20 }}>
              <div style={{ width:64, height:64, borderRadius:16, background:'var(--bg1)', border:'0.5px solid var(--sep)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(235,235,245,0.2)" strokeWidth="1.5">
                  <path d="M2 12c0-5.5 4.5-10 10-10s10 4.5 10 10-4.5 10-10 10S2 17.5 2 12z"/><path d="M12 8v4l3 3"/>
                </svg>
              </div>
              <div style={{ textAlign:'center', maxWidth:440 }}>
                <div style={{ fontSize:15, fontWeight:600, color:'rgba(235,235,245,0.6)', marginBottom:8, letterSpacing:'-0.3px' }}>Canvas Vehicle Analysis</div>
                <div style={{ fontSize:12, color:'rgba(235,235,245,0.25)', lineHeight:1.7 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap', justifyContent:'center' }}>
                {['Sobel','Silhouette','Roofline','Pillars','Classify','Cp field','4-view'].map((s,i,a)=>(
                  <span key={i} style={{ display:'flex', alignItems:'center', gap:5 }}>
                    <span style={{ padding:'3px 10px', borderRadius:6, border:'0.5px solid var(--sep)', fontSize:10, fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.28)', background:'var(--bg1)' }}>{s}</span>
                    {i<a.length-1&&<span style={{ fontSize:10, color:'rgba(235,235,245,0.15)' }}>→</span>}
                  </span>
                ))}
              </div>
            </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, flexShrink:0 }}>
                {VIEWS.map(v=>(
                  <button key={v.id} onClick={()=>setActiveView(v.id)} style={{
                    ...
                    <div style={{ fontSize:10, color: activeView===v.id?'var(--blue)':'rgba(235,235,245,0.3)', textAlign:'center', padding:'3px 0', fontFamily:"'IBM Plex Sans',sans-serif", fontWeight: activeView===v.id?600:400 }}>{v.label}</div>
                  </button>
                ))}
              </div>
            </>
        ...
        {geo ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:5, marginTop:4 }}>
                {BENCHMARKS.map((b,i)=>{const diff=cd-b.Cd,clr=diff<=0?'var(--green)':'var(--orange)';return(
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <span style={{ fontSize:10, fontFamily:"'IBM Plex Mono',monospace", color:'var(--blue)', width:28, flexShrink:0 }}>{b.Cd.toFixed(2)}</span>
                    <span style={{ fontSize:10, color:'rgba(235,235,245,0.35)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{b.name}</span>
                    <span style={{ fontSize:10, fontFamily:"'IBM Plex Mono',monospace", color:clr, flexShrink:0 }}>{diff>0?'+':''}{diff.toFixed(3)}</span>
                  </div>
                )})}
              </div>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:14 }}>
              {[['Front Lift','CL_f',(cd*0.06).toFixed(3),'var(--teal)'],['Rear Lift','CL_r',(-cd*0.02).toFixed(3),'var(--green)'],['Side Force','Cs','0.000','var(--yellow)'],['Drag Force','F_d',(cd*0.5*1.225*40*40*2.4).toFixed(0)+'N','var(--red)']].map(([name,sym,val,clr])=>(
                <div key={name} style={{ background:'var(--bg1)', borderRadius:10, border:'0.5px solid rgba(255,255,255,0.06)', padding:'10px 12px' }}>
                  <div style={{ fontSize:9, fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.25)', marginBottom:5 }}>{name}</div>
                  <div style={{ fontSize:15, fontFamily:"'IBM Plex Mono',monospace", color:clr, fontWeight:600, fontVariantNumeric:'tabular-nums' }}>{val}</div>
                  <div style={{ fontSize:9, fontFamily:"'IBM Plex Mono',monospace", color:'rgba(235,235,245,0.2)', marginTop:2 }}>{sym}</div>
                </div>
              ))}
            </div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {suggestions.map((s,i)=>(
                  <div key={i} style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
                    ...
                    <span style={{ fontSize:12, color:'rgba(235,235,245,0.55)', lineHeight:1.5 }}>{s}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10, padding:'40px 0', textAlign:'center' }}>
            <div style={{ width:48, height:48, borderRadius:12, background:'var(--bg1)', border:'0.5px solid var(--sep)', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(235,235,245,0.2)" strokeWidth="1.5">
                <rect x="3" y="3" width="18" height="18" rx="3"/><path d="M9 9h6M9 12h6M9 15h4"/>
              </svg>
            </div>
            <div style={{ fontSize:12, color:'rgba(235,235,245,0.25)' }}>Upload and analyse a vehicle to see results</div>
          </div>
        )}
      </div>
    </div>
