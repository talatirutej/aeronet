...
export default function Views2DPage() {
  ...
  const analyseVehicle = useCallback(async () => {
    ...
    try {
      ...
      const MAX_CONSECUTIVE_ERRORS = 5; // tolerate brief network suspensions
      for (let i = 0; i < 200; i++) {
        await new Promise(res => setTimeout(res, 3000));
        try {
          // 25s timeout per poll — Vercel free proxy can be slow
          const controller = new AbortController();
          const timer = setTimeout(() => controller.abort(), 25000);
          let data;
          try {
            const resp = await fetch(`${API_BASE}/analyze-contour/result/${job_id}`,
          ...
          if (data.status === "running") {
            ...
            final = data.result; break;
          } else if (data.status === "error") {
            throw new Error(data.error || "Analysis failed");
          }
        } catch (pollErr) {
          if (pollErr.name === "AbortError" || pollErr.message?.includes("suspended") ||
              pollErr.message?.includes("network") || pollErr.message?.includes("fetch")) {
            // Transient network error — retry silently
            consecutiveErrors++;
            setProgressMsg(`Connection issue, retrying… (${consecutiveErrors}/${MAX_CONSECUTIVE_ERRORS})`);
            if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
              throw new Error(`Network lost after ${MAX_CONSECUTIVE_ERRORS} retries: ${pollErr.message}`);
            }
            await new Promise(res => setTimeout(res, 2000)); // wait a bit before retry
            continue;
          }
          throw pollErr; // re-throw real errors (job not found, etc.)
        }
      }
      if (!final) throw new Error("Analysis timeout — check HuggingFace Space status");
