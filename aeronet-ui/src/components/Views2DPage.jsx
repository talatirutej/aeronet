...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── CFD benchmark animation keyframes ── */
  ...
  return (
    <svg viewBox={`0 0 ${CW} ${CH}`} style={{ width: '100%', height: '100%' }} preserveAspectRatio="xMidYMid meet">
      ...
      {(drawDone || isDrawing) && wheels.map((w, i) => (
        /* Dark arch disc — shows the wheel opening without rendering wheel geometry.
           The arch shape is a key CFD measurement; we show the opening, not the wheel. */
        <g key={i} className="wheel-in show" style={{ animationDelay: `${0.9 + i * 0.15}s`, animationFillMode: 'forwards' }}>
          <circle cx={w.cx.toFixed(1)} cy={w.cy.toFixed(1)} r={(w.r * 1.04).toFixed(1)} fill="#070d14" stroke="none"/>
        </g>
      ))}
...
function PipelineOverlay({ pct, msg, sub, visible }) {
  // Ahmed body external flow field — simplified streamlines
  const streamlines = [
  ...
  return (
    <div className={'pl-overlay' + (visible ? '' : ' hidden')}>
      ...
      {/* ── CFD flow field visualisation ─────────────────────────────────── */}
      <div style={{
        position:'relative', width:534, height:242, borderRadius:10, flexShrink:0,
        ...
        <svg width="534" height="242" style={{position:'absolute',inset:0}}>
          ...
          {/* Background mesh — computational domain grid */}
          ...
          {/* Pressure zones — colour-coded Cp */}
          {[
            {cx:196,cy:36,r:26,fill:'rgba(255,75,35,0.22)',dl:0.0},   // stagnation
            {cx:288,cy:26,r:34,fill:'rgba(10,132,255,0.15)',dl:0.4},   // roof suction
            {cx:390,cy:48,r:21,fill:'rgba(10,132,255,0.21)',dl:0.8},   // C-pillar
            {cx:456,cy:110,r:18,fill:'rgba(255,55,55,0.17)',dl:1.2},   // wake
          ].map((z,i) => (
          ...
          {/* Vortex rings — rear separation zone */}
          {[{cx:486,cy:112,r:14,dl:0},{cx:500,cy:145,r:10,dl:0.65}].map((v,i) => (
            <g key={i} style={{
              transformOrigin:`${v.cx}px ${v.cy}px`,
              animation:`cfd-vortex ${3.4+i*0.55}s linear infinite`,
              animationDelay:`${v.dl}s`,
            }}>
              <circle cx={v.cx} cy={v.cy} r={v.r} fill="none"
                stroke="rgba(255,95,45,0.52)" strokeWidth=".9" strokeDasharray="4 3"/>
              <circle cx={v.cx} cy={v.cy} r={v.r*0.48} fill="none"
                stroke="rgba(255,95,45,0.27)" strokeWidth=".7"/>
            </g>
          ))}

          {/* Stagnation point dot */}
          <circle cx="196" cy="36" r="3.8" fill="rgba(255,125,35,0.88)"
            style={{animation:'cfd-pressure 1.7s ease-in-out infinite'}}/>

          {/* Separation dashes */}
          <path d="M476,70 C487,88 492,112 487,132" fill="none"
            stroke="rgba(255,75,55,0.55)" strokeWidth="1.5" strokeDasharray="5 3"
            style={{animation:'cfd-flow 2.1s linear infinite'}}/>

          {/* Scanline sweep */}
          <rect x="0" y="0" width="534" height="20" fill="rgba(10,132,255,0.04)"
            style={{animation:'cfd-scanline 3.5s linear infinite'}}/>

          {/* Domain labels */}
          <text x="12" y="19" fill="rgba(10,132,255,0.40)" fontSize="7"
            fontFamily="'IBM Plex Mono',monospace" letterSpacing=".08em">INLET</text>
          <text x="466" y="19" fill="rgba(10,132,255,0.40)" fontSize="7"
            fontFamily="'IBM Plex Mono',monospace" letterSpacing=".08em">OUTLET</text>
          <text x="12" y="238" fill="rgba(10,132,255,0.28)" fontSize="6"
            fontFamily="'IBM Plex Mono',monospace" letterSpacing=".06em">GROUND PLANE</text>

          {/* Live Cp readouts */}
          {[
            {x:194,y:54, v:'Cp +0.92', c:'rgba(255,105,45,0.78)', dl:0.0},
            {x:284,y:15, v:'Cp −0.41', c:'rgba(10,132,255,0.78)', dl:0.5},
            {x:452,y:98, v:'Cp −0.18', c:'rgba(255,75,55,0.72)',  dl:1.0},
          ].map((d,i) => (
            <text key={i} x={d.x} y={d.y} fill={d.c} fontSize="6.5"
              fontFamily="'IBM Plex Mono',monospace" letterSpacing=".06em"
              style={{animation:'cfd-data 2.1s ease-in-out infinite', animationDelay:`${d.dl}s`}}>
              {d.v}
            </text>
          ))}
        </svg>
        ...
        <div style={{position:'absolute',bottom:7,left:12,right:12}}>
          <div style={{height:2,borderRadius:9999,background:'rgba(255,255,255,0.05)',overflow:'hidden'}}>
            <div style={{height:'100%',borderRadius:9999,
              width:`${pct}%`,
              background:'linear-gradient(90deg,rgba(10,132,255,0.7),rgba(10,220,255,0.9))',
              transition:'width .6s ease'}}/>
          </div>
        </div>
        <div style={{position:'absolute',top:7,right:10,
          fontSize:9,fontFamily:"'IBM Plex Mono',monospace",
          color:'rgba(10,210,255,0.78)',letterSpacing:'.08em'}}>
          {pct}%
        </div>
      </div>
...
export default function AeroNetV2() {
  ...
  // ── Copy outline only ─────────────────────────────────────────────────────
  // Renders the car path alone onto a transparent canvas and copies to clipboard.
  // Nothing inside the outline, nothing outside it — just the white stroke on
  // a transparent background. Paste anywhere (Figma, Slides, etc.).
  const copyOutline = async () => {
    ...
    let dw, dh
    if (bboxAspect > canvasAspect) {
      dw = (CW - PAD*2) * 0.95; dh = dw / bboxAspect
    } else {
      dh = (CH - PAD*2) * 0.90; dw = dh * bboxAspect
      if (dw > (CW - PAD*2) * 0.95) { dw = (CW - PAD*2) * 0.95; dh = dw / bboxAspect }
    }
    ...
    // SVG with only the path — no background, no fill, no wheels
    const svgStr = [
      `<svg xmlns="http://www.w3.org/2000/svg" width="${CW}" height="${CH}" viewBox="0 0 ${CW} ${CH}">`,
      `<path d="${d}" fill="none" stroke="white" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round"/>`,
      `</svg>`,
    ].join('')
    ...
    img.onload = async () => {
      ...
      // No fillRect — canvas starts transparent
      canvas.getContext('2d').drawImage(img, 0, 0)
      canvas.toBlob(async (png) => {
        try {
          await navigator.clipboard.write([new ClipboardItem({ 'image/png': png })])
        } catch {
          // Clipboard API blocked — download instead
          const a = document.createElement('a')
          a.href = URL.createObjectURL(png)
          a.download = 'outline.png'; a.click()
        }
        setCopyDone(true)
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        ...
        <div className="topbar">
          ...
          <button
            className={'tb-btn tb-copy' + (copyDone ? ' copied' : '')}
            onClick={copyOutline}
            disabled={!drawDone}
            title="Copy the car outline as a transparent PNG — no background, no wheels, just the path">
