...
function OutlineThumbnail({ geo, label, isActive, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{ flexShrink:0, width:TW, height:TH, background:'#070d14', borderRadius:5, overflow:'hidden' }}>
                  ...
                  return (
                    <>
                      ...
                      {Object.entries(counts).map(([type, count]) => (
                        <div key={type} style={{ display:'flex', justifyContent:'space-between', fontSize:9, padding:'1.5px 0', borderBottom:'0.5px solid rgba(255,255,255,0.04)' }}>
                          <span style={{ fontFamily:'var(--font-mono)', color:'var(--text-quaternary)' }}>{type}</span>
                          <span style={{ fontFamily:'var(--font-mono)', color: FEAT_COLOR[type] ?? 'var(--blue)', fontWeight:600 }}>
                            {count} detected
                          </span>
                        </div>
                      ))}
                ...
                {geo.arch_wheels?.length > 0 && (
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:9, padding:'1.5px 0', borderBottom:'0.5px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <span style={{ fontFamily:'var(--font-mono)', color:'var(--blue)', fontWeight:600 }}>
                      {geo.arch_wheels.length} wheel{geo.arch_wheels.length !== 1 ? 's' : ''}
                    </span>
                  </div>
                )}
              </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              border:`0.5px solid ${showArches && canShowArches ? 'var(--blue)' : 'rgba(255,255,255,0.1)'}`,
              background: showArches && canShowArches ? 'rgba(10,132,255,0.12)' : 'transparent',
              color: showArches && canShowArches ? 'var(--blue)'
